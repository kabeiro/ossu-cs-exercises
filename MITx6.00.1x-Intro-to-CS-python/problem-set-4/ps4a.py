...
def isValidWord(word, hand, wordList):
    ...
    wordValid = True
    handCopy = hand.copy()
    if len(word) < 1 or word not in wordList:
        wordValid = False
    else:
        for letter in word:
            lettersAvailable = handCopy.get(letter, 0)
            if lettersAvailable < 1:
                wordValid = False
            else:
                handCopy[letter] -= 1
